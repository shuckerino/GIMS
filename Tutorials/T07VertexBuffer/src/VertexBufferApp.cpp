...
class VertexBufferApp : public DX12App
{
private:
  ...
  void createPipeline()
  {
    const auto vertexShader =
        compileShader(L"../../../Tutorials/T07VertexBuffer/Shaders/VertexBuffer.hlsl", L"VS_main", L"vs_6_0");

    const auto pixelShader =
        compileShader(L"../../../Tutorials/T07VertexBuffer/Shaders/VertexBuffer.hlsl", L"PS_main", L"ps_6_0");
  ...
  std::vector<Vertex>    m_vertexBufferCPU = {{{0.0f, 0.25f, 0.0f}}, {{0.25f, -0.25f, 0.0f}}, {{-0.25f, -0.25f, 0.0f}}};
...
public:
  VertexBufferApp(const DX12AppConfig createInfo)
      : DX12App(createInfo)
  {
    createRootSignature();
    createPipeline();
...
int main(int /* argc*/, char /* **argv */)
{
  ...
  config.title    = L"Tutorial 07 Vertex Buffer";
  config.useVSync = true;
  try
  {
    VertexBufferApp app(config);
    app.run();
  }
