...
class TwoConstantBuffers : public DX12App
{
private:
  ...
  void createPipeline()
  {
    const auto vertexShader = compileShader(L"../../../Tutorials/T11TwoConstantBuffers/Shaders/TwoConstantBuffers.hlsl",
                                            L"VS_main", L"vs_6_0");

    const auto pixelShader = compileShader(L"../../../Tutorials/T11TwoConstantBuffers/Shaders/TwoConstantBuffers.hlsl",
                                           L"PS_main", L"ps_6_0");
  ...
  std::vector<ComPtr<ID3D12DescriptorHeap>> m_cbv;
  ...
  void createConstantBuffer()
  {
    ...
    for (ui32 i = 0; i < frameCount; i++)
    {
      ...
      throwIfFailed(getDevice()->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&m_cbv[i])));
      ...
      CD3DX12_CPU_DESCRIPTOR_HANDLE cbvHandle(
          m_cbv[i]->GetCPUDescriptorHandleForHeapStart(), 0,
...
public:
  TwoConstantBuffers(const DX12AppConfig createInfo)
      : DX12App(createInfo)
      , m_counter(0)
  {
    createRootSignature();
  ...
  virtual void onDraw()
  {
    ...
    ID3D12DescriptorHeap* heaps[] = {m_cbv[getFrameIndex()].Get()};

    commandList->SetDescriptorHeaps(1, heaps);

    CD3DX12_GPU_DESCRIPTOR_HANDLE cbvHandle(m_cbv[getFrameIndex()]->GetGPUDescriptorHandleForHeapStart());
    cbvHandle.Offset(0, getDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
...
int main(int /* argc*/, char /* **argv */)
{
  ...
  config.title    = L"Tutorial 11 Two Constant Buffers";
  config.useVSync = true;
  try
  {
    TwoConstantBuffers app(config);
    app.run();
  }
